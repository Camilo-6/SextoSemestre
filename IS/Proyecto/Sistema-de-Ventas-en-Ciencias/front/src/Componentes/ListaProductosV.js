...
function ListaProductosV() {
  ...
  useEffect(() => {
    ...
    fetch("http://127.0.0.1:5000/consulta_producto/lista_vendedor", {
      method: "POST",
      headers: {
  ...
  const handleClic1 = (idProducto) => {
    setCookie("productoM", idProducto, { path: "/" });
    navigate("/modificar_producto");
  };
  ...
  const handleClic3 = (idProducto) => {
    setCookie("productoE", idProducto, { path: "/" });
    navigate("/eliminar_producto");
  };
...
export default ListaProductosV;
